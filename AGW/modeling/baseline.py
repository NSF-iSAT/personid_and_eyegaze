...
from .backbones.resnet_nl import ResNetNL, ResNetNL_new
# from modeling.layer.non_local import Non_local2
from .layer.non_local import Non_local2
...
class Baseline(nn.Module):
    ...
    def __init__(self, num_classes, last_stride, model_path, model_name, gem_pool, pretrain_choice, strategy):
        super(Baseline, self).__init__()
        self.modle_name= model_name

        if model_name == 'resnet50_nl':
        ...
        if self.strategy=="add_nl_layer":
            self. nl_last = Non_local2(2048)

        if self.strategy=="ft_blknl4":
            for name,child in self.base.named_children():
                if 'layer4' not in name and 'NL_4' not in name:
    ...
    def forward(self, x):
        # if self.model_name=='resnet50_nl_mask':
        #     x = self.base(x,weight_mask)
        # else:
        #     x = self.base(x)
        x = self.base(x)
        if self.strategy=="add_nl_layer":
